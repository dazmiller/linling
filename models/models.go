...
// GetLifeInsuranceByProfileId retrieves LifeInsurance by Id. Returns error if
// Id doesn't exist
...
type ProductsLife struct {
	...
	FuneralExpensesBenefit     int       `orm:"column(funeral_expenses_benefit);null"`
	AccidentalDeathBenefit     int       `orm:"column(accidental_death_benefit);null"`
	InterimCover               int       `orm:"column(interim_cover);null"`
	GuaranteedRenewable        int       `orm:"column(guaranteed_renewable);null"`
	TerminalIllnessBenefit     int       `orm:"column(terminal_illness_benefit);null"`
	FinancialPlanningBenefit   int       `orm:"column(financial_planning_benefit);null"`
	FutureInsurability         int       `orm:"column(future_insurability);null"`
	ComlimentaryChildInsurance int       `orm:"column(comlimentary_child_insurance);null"`
	WorldwideCover             int       `orm:"column(worldwide_cover);null"`
	LoyaltyBenefit             int       `orm:"column(loyalty_benefit);null"`
	BenefitIndexation          int       `orm:"column(benefit_indexation);null"`
	PremiumWaver               int       `orm:"column(premium_waver);null"`
	PremiumFreezeOption        int       `orm:"column(premium_freeze_option);null"`
	Active                     int       `orm:"column(active);null"`
	ForExistingCustomers       int       `orm:"column(for_existing_customers);null"`
	UpdatedAt                  time.Time `orm:"column(UpdatedAt);type(datetime);null"`
}

func (t *ProductsLife) TableName() string {
	return "products_life"
}

// AddProductsLife insert a new ProductsLife into database and returns
// last inserted Id on success.
func AddProductsLife(m *ProductsLife) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetProductsLifeById retrieves ProductsLife by Id. Returns error if
// Id doesn't exist
func GetProductsLifeById(id int) (v *ProductsLife, err error) {
	o := orm.NewOrm()
	v = &ProductsLife{Id: id}
	if err = o.Read(v); err == nil {
		return v, nil
	}
	return nil, err
...
// GetAllProductsLife retrieves all ProductsLife matches certain condition. Returns empty list if
// no records exist
func GetAllProductsLife(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(ProductsLife))
	// query k=v
	for k, v := range query {
		// rewrite dot-notation to Object__Attribute
		k = strings.Replace(k, ".", "__", -1)
...
// UpdateProductsLife updates ProductsLife by Id and returns error if
// the record to be updated doesn't exist
func UpdateProductsLifeById(m *ProductsLife) (err error) {
	o := orm.NewOrm()
	v := ProductsLife{Id: m.Id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Update(m); err == nil {
...
// DeleteProductsLife deletes ProductsLife by Id and returns error if
// the record to be deleted doesn't exist
func DeleteProductsLife(id int) (err error) {
	o := orm.NewOrm()
	v := ProductsLife{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&ProductsLife{Id: id}); err == nil {
			fmt.Println("Number of records deleted in database:", num)
		}
	}
	return
...
func init() {
	orm.RegisterModel(new(LifeInsurance), new(FuneralInsurances), new(HomeInsurances), new(MortgageInsurances), new(Providers), new(Products), new(QuoteMessages), new(QuoteContactRequests), new(Profile), new(ProductsLife))
}
